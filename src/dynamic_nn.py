...
def nn_heuristic(av_req, av_taxis, battery_level, matrix_task,
    ...
    for i in range(len(av_req_copy)):
        ...
        av_req_loc = {i : [data.ori_x[data.index[data['req_id'] == i].tolist()[0] ],
                    data.ori_y[ data.index[data['req_id'] == i].tolist()[0]] ] for
                    i in av_req.keys()}
        
        # distances between the current taxi and requests locations
        distances = {i: round(math.sqrt((av_req_loc[i][0]-current_taxi_loc[0])**2 +
                    (av_req_loc[i][1]-current_taxi_loc[1])**2), 2) for i in av_req.keys()}
...
if __name__ == "__main__":
    ...
    window_lengths = [10]#, 90, 180, 240, 360, 480, 720, 840, 1440] 
    ...
    instances = ["new_instances/instance100_3.txt"]
    ...
    for window_len in window_lengths:
        ...
        for instance_name in instances:
            ...
            nn_cpu, nn_cumul_obj_val, nn_serv_req, nn_unserv_req, nn_matrix_task, nn_matrix_start_time, nn_matrix_fini_time = uf.rolling_time_window(nn_heuristic,
                                                                                                                        T_inf,T_sup,
                                                                                                                        window_len,
                                                                                                                        req, av_taxis,
                                                                                                                        data)
            ...
            print("* percentage of unserved requests : ", 
                  round(len(nn_unserv_req)/len(data),2)*100, " %") 
